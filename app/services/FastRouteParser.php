...
use Exception;
use Framework\services\ParserInterface;
    ...
    public function parse($route)
    {
        ...
        if ($optionCount !== count($segments)-1) {
            ...
            if (preg_match('~' . self::VARIABLE_REGEX . '(*SKIP)(*F) | \]~x', $routeWithoutClosingOptionals)) {
                throw new Exception("Optional segments can only occur at the end of a route", $route);
            }
            throw new Exception("Number of opening '[' and closing ']' does not match", $route);
        }
        ...
        foreach ($segments as $n=>$segment) {
            if ($segment === '' && $n !== 0) {
                throw new Exception("Empty optional part", $route);
            }
